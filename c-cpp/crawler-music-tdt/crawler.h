...
void writeTime()
{
    ...
    string currTime("");
   /* sprintf(currTime, "GET /%s HTTP/1.1\r\nHost: %s:%d\r\nIf-Modified-Since:%s, %d %s %d %d:%d:%d GMT\r\nConnection: Close\r\n\r\n", file, host, port, wday[p->tm_wday], p->tm_mday, month[p->tm_mon], (1900 + p->tm_year), (p->tm_hour+8 - 1), p->tm_min, p->tm_sec);
*/
   currTime += wday[p->tm_wday];
   currTime += ", ";
   if(p->tm_mday < 10)
       currTime += "0";
   currTime += integer2string(p->tm_mday);
   currTime += " ";
   currTime += month[p->tm_mon]; 
   currTime += " ";
   currTime += integer2string(1900 + p->tm_year);
   currTime += " ";
   if((p->tm_hour+8) < 10)
       currTime += "0";
   currTime += integer2string(p->tm_hour+8);
   currTime += ":";
   if((p->tm_min) < 10)
       currTime += "0";
   currTime += integer2string(p->tm_min);
   currTime += ":";
   if((p->tm_sec) < 10)
       currTime += "0";
   currTime += integer2string(p->tm_sec);
   currTime += " GMT";

   fstream timefile;
   timefile.open("last_fetch.time", ios::out|ios::trunc);
   if(!timefile.is_open())
   {
       cout << "can't open time file" << endl;
       exit(1);
   }
   timefile << currTime;
   timefile.close();
...
int crawler()
{
    ...
    cout << "========================" << endl;
    crawl::initUrlSeen();
    crawl::initURLFrontier();
    crawl::initContentSeen();
    cout << "========================" << endl;
