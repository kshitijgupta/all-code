...
bool HttpModule::fetch( struct page_link * ppl, char* buffer, char* bigbuf, int tid) {
...
if ((res = pthread_kill(thread, SIGALRM)) == 0) {
        sleep(1);
        printf("＠抓取线程超时,pthread_cancle success!\n");

} else {
        sleep(1);
	printf("＠抓取线程超时,pthread_cancle failed!\n");
...
void* HttpModule::threadtofetch(void* p_attr) {
	...
	struct threadPara * pt = (struct threadPara *) p_attr;
        ...
        //for debug
        char* pos1 = (char*)pt->ppl->_host_addr;

	char * domain = pt->ppl->_host_addr;
...
tp.print("ip: %s\n", ip);
	...
	server_addr.sin_port = htons(pt->ppl->_portnumber);
...
*/
        ...
        snprintf(
                ...
                Host: %s:%d\r\nConnection: Close\r\n\r\n",
                pt->ppl->_host_file, pt->ppl->_host_addr, pt->ppl->_portnumber);
#ifdef DEBUG
        tp.print("%s\n", request);
#endif
        request[2047] = '\0';
 ...
 //for debug
        char* pos2 = (char*)pt->ppl->_host_file;
...
#endif
 //for debug
        char* pos3 = (char*)pt->ppl->_portnumber;
...
void* thread_LaunchMemberFunction(void*obj) {
    ...
    struct Wrapper_t*const wt = (Wrapper_t*) obj;

    ((HttpModule *const) (wt->hm))->threadtofetch(wt->tp);
    pthread_exit(NULL);
}
