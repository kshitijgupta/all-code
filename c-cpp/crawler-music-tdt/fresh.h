...
#include "util/rabinhash-32/rabinhash-c++.h"
...
void conditional_get_pkg(char* request, int size, char* t, char* host, char* file, int port)
{
...
bool is_modified(struct page_link *ppl, char* t){
    ...
    conditional_get_pkg(request, 2048,t,  ppl->_host_addr, ppl->_host_file, ppl->_portnumber);

    if((hostent = gethostbyname(ppl->_host_addr)) == NULL)
    {
    ...
    server_addr.sin_port   = htons(ppl->_portnumber);
    ...
    char buffer[1024];
    int pos=0;
    while((nbytes = read(sockfd, &buffer[pos], 1)) == 1)
    {
        if(i < 4)
        {
    ...
    char* pm = strstr(buffer, "304 Not Modified");
    if(pm != NULL)
        return true;
    else
        return false;
...
//printf("好使的host:%s   file:%s       hash:%lld\n", holder_pl->_host_addr, holder_pl->_host_file, urlhash);
...
{
    fstream timefile;
    timefile.open("last_fetch.time", ios::in);
    if(!timefile.is_open()){
        cout << "Can't open last_fetch.time" << endl;
        exit(0);
    }
    timefile.getline(s, 128);
    timefile.close();
}
void fresh()
{
    ...
    bool newpage = false;//如果没有任何更新直接退出
    while (seedfile.getline(httpbuffer, 1280)) {
        ...
        if (le.URLanalyser(&main_pl, httpbuffer)&&is_modified(&main_pl, lastTime)) {
    ...
    cout << "Last fetch time : " << lastTime << endl;

    if(!newpage){
        cout << "Nothing new, I will exit" <<endl;
        exit(0);
    }
    ...
    for (int t = 0; t < MAX_THREAD; t++) {
        ...
        rc = pthread_create(&threads[t], NULL, update_thread, (void*) t);
        if (rc) {
