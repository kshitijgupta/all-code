...
ssize_t rio_readn(int fd, void* usrbuf, size_t n) {
	...
	while (nleft > 0) {
		...
		if ((nread = read(fd, bufp, nleft)) < 0) {
			if (errno== EINTR){
				if(debug)std::cout << "error0原因：" << strerror(errno) << std::endl;
				nread = 0;}
			else{
				if(debug)std::cout << "error1原因：" << strerror(errno) << std::endl;
				return -1;}
		} else if (nread == 0) {
			break;
		}
...
ssize_t rio_readn_chunked(int fd, void*usrbuf, size_t n){
    ...
    while(chunksize != 0)
    {
        //如果要读入的数据量大于允许最大量，或者要求读入chunksize大小的文件而读到的小于chunksize返回错误
        if (((nleft = nleft - chunksize)<0)||((nread = rio_readn(fd, p, chunksize)) != chunksize))
        {
            return -1;
        }
