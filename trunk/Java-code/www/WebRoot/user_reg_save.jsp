...
<html>
	...
	<%
		...
		{	
			...
			else
			{
				...
				String sUserPassword = request.getParameter("UserPassword");
                                String CUserPassword = request.getParameter("CUserPassword");
                              if(sUserPassword.equals(CUserPassword))
                              {
				...
				if(User.add())
				{
					...
					out.println("<p><font color=red>用户注册失败，请稍后重试</font></p>");
                                    ...
                                    {
                                    out.println("<p><font color=red>两次输入密码不同</font></p>");
                                    }
		...
		//输出跳转目的页面信息
		if(isSuc)
			out.println("<p><input type=button name=btn value=\"&nbsp;登录&nbsp;\" onClick=\"window.location='bbs_index.html'\"></p>");
		else
