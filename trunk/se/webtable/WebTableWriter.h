 ...
 *
 *
 * 在实际使用中，应该为为每个线程／进程最多创建一个WebTableWriter对象，可以反复使用write方法。
 *
 * 注意当前版本的实现中，网络传输数据未采用压缩算法，考虑到爬虫如果直接安排在HBase机器上跑,压缩会影响爬虫效率，在数据库存储端安排了压缩
 * 如果爬虫需要独立机器应该修改代码保证在爬虫端压缩，而数据库端不必压缩
 * */
