...
//		}
        ...
        else if(Mode == 'c')
        {
            ...
            {   // assume the Btree is empty if you cannot read from the file
                Error("File cannot read CurrentNode");
            }
            else   // Node zero is not a normal node, it contains the following:
            {
                NumItems = CurrentNode.Branch[0];
...
void BTTableClass::getCurrentNode(NodeType &nt){
...
void BTTableClass::getRoot(long& root){
    root = Root;
}
...
void BTTableClass::getNumNodes(long& size){
    size = NumNodes;
}
void BTTableClass::setNumItems(long size){
    NumItems = size;
}
void BTTableClass::getNumItems(long& size){
    size = NumItems;
}
