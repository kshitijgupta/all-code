...
//luolei
static void 
printRowVector(const std::vector<TRowResult> trsv)
{
	for(int i=0; i<trsv.size(); i++){
		TRowResult rowResult = trsv[i];
  ...
  try {
    ...
    for (int i = 100; i >= 0; --i) {
     ...
     // TRowResult rowResult;
      std::vector<TRowResult> rowResultvector;
      ...
      mutations.push_back(Mutation());
      mutations.back().column = "entry:num";
      mutations.back().value = lexical_cast<std::string>(i);
      mutations.push_back(Mutation());
      mutations.back().column = "entry:sqr";
      mutations.back().value = lexical_cast<std::string>(i*i);
      client.mutateRow(t, row, mutations);
      client.getRow(rowResultvector, t, row);
      printRowVector(rowResultvector);
